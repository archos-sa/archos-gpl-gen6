...
#
#############################################################
LIBGLIB2_VERSION:=2.16.5
LIBGLIB2_SOURCE:=glib-$(LIBGLIB2_VERSION).tar.bz2
LIBGLIB2_SITE:=http://ftp.gnome.org/pub/gnome/sources/glib/2.16
LIBGLIB2_CAT:=$(BZCAT)
LIBGLIB2_DIR:=$(BUILD_DIR)/glib-$(LIBGLIB2_VERSION)
LIBGLIB2_BINARY:=libglib-2.0.a
LOCAL_GLIB_GENMARSHAL:=$(shell which glib-genmarshal)

ifeq ($(BUILD),DEBUG)
GLIB_TARGET_CONFIGURE_OPTS+=$(TARGET_CONFIGURE_OPTS) CFLAGS='-O2 -I$(LIBGLIB2_DIR)/../libiconv-1.8/include'
#GLIB_TARGET_CONFIGURE_OPTS+=$(TARGET_CONFIGURE_OPTS:-O%=) CFLAGS='-g -O0 -I$(LIBGLIB2_DIR)/../libiconv-1.8/include'
#TARGET_CFLAGS=-g -O0
else
GLIB_TARGET_CONFIGURE_OPTS+=$(TARGET_CONFIGURE_OPTS) CFLAGS='-O2 -I$(LIBGLIB2_DIR)/../libiconv-1.8/include'
endif
...
GETTEXT_MAGIC = gt_cv_func_ngettext_libc=yes gt_cv_func_dgettext_libc=yes
ICONV_MAGIC   = --with-libiconv=gnu
libglib2_iconv = iconv
else
GETTEXT_MAGIC = 
ICONV_MAGIC   = --with-libiconv=gnu
libglib2_iconv = gettext libintl iconv
endif

ifeq ($(BUILD),DEBUG)
ENABLE_DEBUG=--enable-debug=yes --disable-visibility
else
ENABLE_DEBUG=
endif
...
$(LIBGLIB2_DIR)/.configured: $(LIBGLIB2_DIR)/.unpacked
	(cd $(LIBGLIB2_DIR); rm -rf config.cache; \
		...
		gl_cv_c_restrict=no \
		ac_cv_path_GLIB_GENMARSHAL=$(LOCAL_GLIB_GENMARSHAL) \
		ac_cv_prog_F77=no \
		ac_cv_prog_CXX=no \
		$(GETTEXT_MAGIC) \
		./configure \
...
$(STAGING_DIR)/lib/$(LIBGLIB2_BINARY): $(LIBGLIB2_DIR)/glib/.libs/$(LIBGLIB2_BINARY)
	...
	# don't install the native binary - it only confuses libtinymail...
	-rm -f $(STAGING_DIR)/usr/bin/glib-genmarshal
...
libglib2: uclibc $(libglib2_iconv) pkgconfig $(TARGET_DIR)/lib/libglib-2.0.so.0.1200.6
